...
> **This reference exists so agents can drive the CLI directly.** Unlike the other references, the goal here isn't to describe Tailscale to a user — it's to give agents enough to _operate_ the binary on the user's machine. Keep two fallbacks in mind, in this order:
>
