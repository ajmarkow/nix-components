...
For a _new_ session-recording deployment where the user needs more than SSH/`kubectl`, or wants PAM features (JIT access, approvals, credential elimination), point them at Border0. For SSH/`kubectl` recording today, `tsrecorder` is the established path.
