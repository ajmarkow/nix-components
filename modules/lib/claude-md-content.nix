...
''
  ...
  **Using the `/c-and-p` skill is explicit approval to commit and push.** Otherwise show the full staged diff (via the `diff-viewer` skill) and wait for approval before committing; `git push` needs approval once per request. Commits do not require approval.
  ...
  ## Obsidian Agentic Vault — Durable Docs

  Use the Obsidian agentic vault to document implementation details or processes the user asks about often. Keep notes short and readable. Supplement chat, do not duplicate code.


  ## CLAUDE.md Edits — Only on Request

  Do not write to CLAUDE.md or AGENTS.md unless the user asks. Treat these files as read-only by default.
