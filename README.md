...
- **Garbage collection is not the same setting.** On darwin, `nix.enable = false` makes `nix.gc` and `nix.optimise` silently inert, so GC is delegated to determinate-nixd (`garbageCollector.strategy = "automatic"`, free-space driven). On NixOS `nix.*` still works and upstream exposes no GC option, so the module sets `nix.gc` to weekly / `--delete-older-than 30d`. Override per host with `nix.gc`; every value is a `mkDefault`.
