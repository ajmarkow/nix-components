...
**Use Backlog.md only when the user explicitly invokes the `backlog-md` skill or directly asks to use Backlog.md or the `backlog` CLI. Do not infer its use from generic task management, planning, search, implementation, or status requests.**

For an explicit Backlog.md request, run `backlog instructions overview` first.
